    ...
    def _exec_kachaka(self, log_manager, date, ip) -> None:
        ...
        while (
            ...
            if self._dis_pre[0] == -1:
                self._dis_pre[0] = 0
                for j in range(3):
                    self._dis_pre[0] += rewards[0][0][5][j]
            for j in self._target_index_list[0]:
                dis += rewards[0][0][4][j-maps.MAP_TARGET_POINT_INDICATOR]
            ...
            if ci[0] != -sys.float_info.max:
                ...
                ci[0] = self._do_take_picture_object(top_map, fog_of_war_map, n)
                ...
                if len(cover_list) == 0:
                    ...
                    if len(self._taken_picture_list[0]) != self._num_picture:
                        ...
                        if len(self.config.VIDEO_OPTION) > 0:
                            self._taken_picture[0].append(observations[n]["rgb"])
                        reward[0] += ci[0]
                    ...
                    #範囲が多く被っていなくて、self._num_picture回以上写真を撮っていたら
                        ...
                        if idx != -1:
                            ...
                            if len(self.config.VIDEO_OPTION) > 0:
                                self._taken_picture[0][idx] = observations[0]["rgb"]   
                ...
                # 1つとでも多く被っていた時    
                    ...
                    if self._compare_with_changed_CI(picture_range_map, pre_fog_of_war_map, cover_list, ci[0], min_ci_k, min_idx) == True:
                        ...
                        if len(self.config.VIDEO_OPTION) > 0:
                            self._taken_picture[0][min_idx] = observations[0]["rgb"]   
            ...
            if not_done_masks[0].item() == 0:                    
                ...
                if len(self.config.VIDEO_OPTION) > 0:
                    if len(rgb_frames[0]) == 0:
                        frame = observations_to_image(observations[0], infos[0], action[0].cpu().numpy())
                    ...
                    for j in range(len(self._taken_picture_list[0])):
                        ...
                        picture_name = "episode=" + str(current_episodes[0].episode_id)+ "-ckpt=" + str(checkpoint_index) + "-" + str(j) + "-" + str(name_p)
